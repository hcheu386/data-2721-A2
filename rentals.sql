...
call avaible_apt('2025-01-01', '2025-01-31');

-- List the buildings with the highest number of booked apartments.
delimiter $
create procedure most_booked_buildings()
begin

    declare max_bookings int;

    select
        max(bookings) into max_bookings
    from (
        select
            count(l.apartment) as bookings
        from building b
